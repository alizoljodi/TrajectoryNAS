...
KinematicsData = Tuple[
    float, float, float, float, float, float, float, float, float, float
]


def _kinematics_from_tokens(
    helper: PredictHelper, instance: str, sample: str
) -> KinematicsData:
    """
...
def _constant_velocity_heading_from_kinematics(
    kinematics_data: KinematicsData, sec_from_now: float, sampled_at: int
) -> np.ndarray:
    """
...
def _constant_acceleration_and_heading(
    kinematics_data: KinematicsData, sec_from_now: float, sampled_at: int
) -> np.ndarray:
    """
    ...
    for time in np.arange(time_step, sec_from_now + time_step, time_step):
        ...
        preds.append(
            (
                x + time * vx + half_time_squared * ax,
...
def _constant_speed_and_yaw_rate(
    kinematics_data: KinematicsData, sec_from_now: float, sampled_at: int
) -> np.ndarray:
    """
    Computes a baseline prediction for the given time window and frequency, under
...
def _constant_magnitude_accel_and_yaw_rate(
    kinematics_data: KinematicsData, sec_from_now: float, sampled_at: int
) -> np.ndarray:
    """
    Computes a baseline prediction for the given time window and frequency, under
...
class Baseline(abc.ABC):
    ...
    def __init__(self, sec_from_now: float, helper: PredictHelper):
        ...
        assert (
            sec_from_now % 0.5 == 0
        ), f"Parameter sec from now must be divisible by 0.5. Received {sec_from_now}."
        self.helper = helper
...
class ConstantVelocityHeading(Baseline):
    ...
    def __call__(self, token: str) -> Prediction:
        ...
        cv_heading = _constant_velocity_heading_from_kinematics(
            kinematics, self.sec_from_now, self.sampled_at
        )

        # Need the prediction to have 2d.
        return Prediction(
            instance, sample, np.expand_dims(cv_heading, 0), np.array([1])
        )
...
class PhysicsOracle(Baseline):
    ...
    def __call__(self, token) -> Prediction:
        ...
        ground_truth = self.helper.get_future_for_agent(
            instance, sample, self.sec_from_now, in_agent_frame=False
        )

        assert ground_truth.shape[0] == int(self.sec_from_now * self.sampled_at), (
            "Ground truth does not correspond " f"to {self.sec_from_now} seconds."
        )
        ...
        paths = [
            path_fun(kinematics, self.sec_from_now, self.sampled_at)
            for path_fun in path_funs
        ]
        ...
        oracle = sorted(
            paths,
            key=lambda path: np.linalg.norm(np.array(path) - ground_truth, ord="fro"),
        )[0]
