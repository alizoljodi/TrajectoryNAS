...
from nuscenes.prediction.models.covernet import (
...
class TestCoverNet(unittest.TestCase):
    ...
    def test_shapes_in_forward_pass_correct(self):
        ...
        covernet = CoverNet(
            resnet, 5, n_hidden_layers=[4096], input_shape=(3, 100, 100)
        )
...
class TestConstantLatticeLoss(unittest.TestCase):
    ...
    def test_constant_lattice_loss(self):
        ...
        logits = torch.Tensor(
            [[2, 10, 5], [-3, 4, 5], [-4, 2, 7], [8, -2, 3], [10, 3, 6]]
        )
        ...
        self.assertAlmostEqual(
            float(loss_value.detach().numpy()), float(answer.detach().numpy())
        )
