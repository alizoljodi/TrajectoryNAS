...
class TestUtils(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.left_path = {
            "start_pose": [391.7142849867393, 1100.464077182952, 2.7365754617298705],
            ...
            "segment_length": [
                22.380622583127813,
                0.18854612175175053,
                0.0010839266609007578,
            ],
        }
        self.right_path = {
            "start_pose": [367.53376358458553, 1097.5300417399676, 1.1738120532326812],
            "end_pose": [392.24904359636037, 1112.5206834496375, -0.4033046016493182],
            "shape": "RSR",
            "radius": 16.890467008945414,
            "segment_length": [
                4.423187697943063e-05,
                6.490596454713637,
                26.63819259666578,
            ],
        }
    ...
    def test_length_of_lane(self):
        ...
        self.assertEqual(
            arcline_path_utils.length_of_lane(self.straight_lane),
            sum(self.straight_path["segment_length"]),
        )

        self.assertEqual(
            arcline_path_utils.length_of_lane(self.right_lane),
            sum(self.right_path["segment_length"]),
        )

        self.assertEqual(
            arcline_path_utils.length_of_lane(self.curved_lane),
            sum(self.straight_path["segment_length"])
            + sum(self.left_path["segment_length"]),
        )
    ...
    def test_project_pose_to_straight_lane(self):
        ...
        end_pose = (
            421.2419602954602 + 10 * math.cos(theta),
            1087.9127960414617 + 10 * math.sin(theta),
            theta,
        )
        ...
        np.testing.assert_allclose(
            np.array(pose).astype("int"), np.array(end_pose).astype("int")
        )
    ...
    def test_project_pose_to_curved_lane(self):
        ...
        np.testing.assert_allclose(
            np.array(pose).astype("int"), np.array(end_pose_1).astype("int")
        )
        self.assertTrue(abs(s - 10) <= 0.5)

        pose_2, s_2 = arcline_path_utils.project_pose_to_lane(
            end_pose_2, self.curved_lane
        )
        np.testing.assert_allclose(
            np.array(pose_2[:2]).astype("int"), np.array([380, 1100])
        )
        self.assertTrue(abs(s_2 - 44) <= 0.5)

    def test_get_curvature_straight_lane(self):

        curvature = arcline_path_utils.get_curvature_at_distance_along_lane(
            15, self.straight_lane
        )
