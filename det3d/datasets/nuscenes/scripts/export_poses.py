...
def derive_latlon(
    location: str, poses: List[Dict[str, float]]
) -> List[Dict[str, float]]:
    """
...
def export_kml(
    coordinates_per_location: Dict[str, Dict[str, List[Dict[str, float]]]],
    output_path: str,
) -> None:
    """
...
def main(
    dataroot: str, version: str, output_prefix: str, output_format: str = "kml"
) -> None:
    """
    ...
    for scene in tqdm(nusc.scene):
        ...
        location = nusc.get("log", scene["log_token"])[
            "location"
        ]  # Needed to extract the reference coordinate.
        poses = get_poses(
            nusc, scene_token
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Export ego pose coordinates from a scene to a .json file.",
    ...
    parser.add_argument(
        "--output_format", type=str, default="kml", help="Output format (kml or json)."
    )
