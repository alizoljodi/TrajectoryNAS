...
def export_scene_pointcloud(
    ...
    with open(out_path, "w") as f:
        ...
        for sd_token in tqdm(sd_tokens):
            ...
            if verbose:
                print(
                    "Distance filter: Keeping %d of %d points..."
                    % (keep.sum(), len(keep))
                )
...
def pointcloud_color_from_image(
    nusc: NuScenes, pointsensor_token: str, camera_token: str
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Export a scene in Wavefront point cloud format.",
    ...
    parser.add_argument(
        ...
        default="scene-0061",
        type=str,
        help="Name of a scene, e.g. scene-0061",
    )
    parser.add_argument(
        "--out_dir",
        default="~/nuscenes-visualization/pointclouds",
        type=str,
        help="Output folder",
    )
    parser.add_argument(
        "--verbose", default=0, type=int, help="Whether to print outputs to stdout"
    )
    ...
    export_scene_pointcloud(
        nusc, out_path, scene_tokens[0], channel="LIDAR_TOP", verbose=verbose
    )
