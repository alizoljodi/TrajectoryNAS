...
def compute_metrics(
    predictions: List[Dict[str, Any]], helper: PredictHelper, config: PredictionConfig
) -> Dict[str, Any]:
    """
    ...
    for i, prediction_str in enumerate(predictions):
        ...
        ground_truth = helper.get_future_for_agent(
            prediction.instance, prediction.sample, config.seconds, in_agent_frame=False
        )
...
def main(
    ...
    json.dump(
        results, open(submission_path.replace(".json", "_metrics.json"), "w"), indent=2
    )
