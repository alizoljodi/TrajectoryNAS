...
class TestFunctions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.x_many_batches_and_modes = np.repeat(
            np.expand_dims(self.x_many_modes, 0), 5, axis=0
        )
        self.y_many_batches_and_modes = np.repeat(
            np.expand_dims(self.y_many_modes, 0), 5, axis=0
        )
        self.p_many_batches_and_modes = np.array(
            [
                [0.2, 0.5, 0.3],
                [0.5, 0.3, 0.2],
                [0.2, 0.3, 0.5],
    ...
    def test_mean_distances_many_modes(self):
        ...
        np.testing.assert_allclose(
            value, np.array([[6.45396, 5.33529, 4.49286]]), atol=1e-4, rtol=1e-4
        )

    def test_mean_distances_many_batches_and_modes(self):
        value = metrics.mean_distances(
            self.x_many_batches_and_modes, self.y_many_batches_and_modes
        )
        np.testing.assert_allclose(
            value, np.array(5 * [[6.45396, 5.33529, 4.49286]]), atol=1e-4, rtol=1e-4
        )
    ...
    def test_max_distances_many_modes(self):
        ...
        np.testing.assert_allclose(
            value, np.array([[12.04159, 10.63014, 9.21954]]), atol=1e-4, rtol=1e-4
        )

    def test_max_distances_many_batches_and_modes(self):
        value = metrics.max_distances(
            self.x_many_batches_and_modes, self.y_many_batches_and_modes
        )
        np.testing.assert_allclose(
            value, np.array(5 * [[12.04159, 10.63014, 9.21954]]), atol=1e-4, rtol=1e-4
        )
    ...
    def test_miss_max_distances_many_batches_and_modes(self):
        value = metrics.miss_max_distances(
            self.x_many_batches_and_modes, self.y_many_batches_and_modes, 10
        )
        np.testing.assert_equal(value, np.array(5 * [[True, True, False]]))

    def test_miss_rate_top_k_one_mode(self):
        value = metrics.miss_rate_top_k(
            self.x_one_mode, self.y_one_mode, self.p_one_mode, 2
        )
        np.testing.assert_equal(value, np.array([[True]]))

    def test_miss_rate_top_k_many_modes(self):
        value = metrics.miss_rate_top_k(
            self.x_many_modes, self.y_many_modes, self.p_many_modes, 10
        )
        np.testing.assert_equal(value, np.array([[True, False, False]]))

    def test_miss_rate_top_k_many_batches_and_modes(self):
        value = metrics.miss_rate_top_k(
            self.x_many_batches_and_modes,
            self.y_many_batches_and_modes,
    ...
    def test_min_ade_k_many_modes(self):
        value = metrics.min_ade_k(
            self.x_many_modes, self.y_many_modes, self.p_many_modes
        )
        np.testing.assert_allclose(
            value, np.array([[5.33529, 4.49286, 4.49286]]), atol=1e-4, rtol=1e-4
        )
    ...
    def test_min_fde_k_many_modes(self):
        value = metrics.min_fde_k(
            self.x_many_modes, self.y_many_modes, self.p_many_modes
        )
        np.testing.assert_allclose(
            value, np.array([[10.63014, 9.21954, 9.21954]]), atol=1e-4, rtol=1e-4
        )
    ...
    def test_min_fde_k_many_batches_and_modes(self):
        ...
        np.testing.assert_allclose(
            ...
            np.array(
                [
                    [10.63014, 9.21954, 9.21954],
                    [12.04159, 10.63014, 9.21954],
                    [9.21954, 9.21954, 9.21954],
                    [9.21954, 9.21954, 9.21954],
                    [10.63014, 10.64014, 9.21954],
                ]
            ),
            atol=1e-3,
            rtol=1e-3,
    ...
    def test_desired_number_of_modes_enough_data(self):
        ...
        value = metrics.desired_number_of_modes(results, [1, 5, 15, 25])
        np.testing.assert_equal(
            value, np.array([[0, 4, 14, 24], [25, 29, 39, 49], [50, 54, 64, 74]])
        )

    def test_desired_number_of_modes_not_enough(self):
        results = np.arange(30).reshape(2, 15)
        value = metrics.desired_number_of_modes(results, [1, 5, 15, 25])
        np.testing.assert_equal(value, np.array([[0, 4, 14, 14], [15, 19, 29, 29]]))
...
class TestMetrics(unittest.TestCase):
    ...
    def test_OffRoadRate(self):
        with patch.object(metrics.OffRoadRate, "load_drivable_area_masks"):
            ...
            self.assertDictEqual(
                off_road_rate.serialize(),
                {"name": "OffRoadRate", "aggregators": [{"name": "RowMean"}]},
            )
    ...
    def test_deserialize_metric(self):
        ...
        with patch.object(metrics.OffRoadRate, "load_drivable_area_masks"):
            config = {"name": "OffRoadRate", "aggregators": [{"name": "RowMean"}]}
    ...
    def test_flatten_metrics(self):
        ...
        metric_functions = [
            ...
            metrics.MissRateTopK(
                [1, 5, 10], tolerance=2, aggregators=[metrics.RowMean()]
            ),
        ]
...
class TestOffRoadRate(unittest.TestCase):
    ...
    def _do_test(self, map_name, predictions, answer):
        with patch.object(
            PredictHelper, "get_map_name_from_sample_token"
        ) as get_map_name:
            ...
            prediction = Prediction(
                "foo-instance", "foo-sample", predictions, probabilities
            )

            # Two violations out of three trajectories
            np.testing.assert_allclose(
                off_road_rate(np.array([]), prediction), np.array([answer])
            )
    ...
    def test_one_north(self):
        predictions = np.array(
            [
                [
                    [965.8515334916171, 535.711518726687],
    ...
    def test_queenstown(self):
        predictions = np.array(
            [
                [
                    [744.8769428947988, 2508.398411382534],
    ...
    def test_hollandvillage(self):
        predictions = np.array(
            [
                [
                    (1150.811356677105, 1598.0397224872172),
