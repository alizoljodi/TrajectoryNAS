...
def visualize_sample(
    ...
    pc, _ = LidarPointCloud.from_file_multisweep(
        nusc, sample_rec, "LIDAR_TOP", "LIDAR_TOP", nsweeps=20
    )
...
def visualize_sample_forecast(
    ...
    for box, center, trajectory in zip(boxes_gt, center_gt, gt_trajectory):
        box.render_forecast(
            ax, view=np.eye(4), colors=("g", "g", "g"), linewidth=2, center=center
        )

    # Show EST boxes.
    for box, center, clr, trajectory in zip(
        boxes_est, center_est, color, est_trajectory
    ):
        if "r" in clr:
            continue
...
def summary_plot(
    ...
    for ind, detection_name in enumerate(getDetectionNames(cohort_analysis)):
        ...
        try:
            ...
            ax2 = setup_axis(
                xlim=1, title=title2, min_recall=min_recall, ax=axes[ind, 1]
            )
