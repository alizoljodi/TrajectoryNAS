...
class DetectionMetricData(MetricData):
    ...
    def serialize(self):
        ...
        return {
            ...
            "forecast_precision": [
                self.forecast_precision[i].tolist()
                for i in range(len(self.forecast_precision))
            ],
    ...
    def deserialize(cls, content: dict):
        ...
        return cls(
            ...
            forecast_precision=[
                np.array(content["forecast_precision"][i])
                for i in range(len(content["forecast_precision"]))
            ],
...
class DetectionMetrics:
    ...
    def add_label_fap_mr(
        self, detection_name: str, dist_th: float, fap_mr: float
    ) -> None:
    ...
    def mean_dist_faps_mr(self) -> Dict[str, float]:
        ...
        return {
            class_name: np.mean(list(d.values()))
            for class_name, d in self._label_faps_mr.items()
        }
    ...
    def mean_dist_ars(self) -> Dict[str, float]:
        ...
        return {
            class_name: np.mean(list(d.values()))
            for class_name, d in self._label_ars.items()
        }
    ...
    def mean_dist_faps(self) -> Dict[str, float]:
        ...
        return {
            class_name: np.mean(list(d.values()))
            for class_name, d in self._label_faps.items()
        }
    ...
    def mean_dist_fars(self) -> Dict[str, float]:
        ...
        return {
            class_name: np.mean(list(d.values()))
            for class_name, d in self._label_fars.items()
        }
    ...
    def mean_dist_aaps(self) -> Dict[str, float]:
        ...
        return {
            class_name: np.mean(list(d.values()))
            for class_name, d in self._label_aaps.items()
        }
    ...
    def mean_dist_aars(self) -> Dict[str, float]:
        ...
        return {
            class_name: np.mean(list(d.values()))
            for class_name, d in self._label_aars.items()
        }
    ...
    def deserialize(cls, content: dict):
        ...
        for detection_name, label_ars in content["label_ars"].items():
            for dist_th, ar in label_ars.items():
                metrics.add_label_ar(
                    detection_name=detection_name, dist_th=float(dist_th), ar=float(ar)
                )

        for detection_name, label_faps in content["label_faps"].items():
            for dist_th, fap in label_faps.items():
                metrics.add_label_fap(
                    detection_name=detection_name,
                    dist_th=float(dist_th),
                    fap=float(fap),
                )
...
class DetectionBox(EvalBox):
    ...
    def deserialize(cls, content: dict):
        ...
        return cls(
            ...
            forecast_score=(
                -1.0
                if "forecast_score" not in content
                else float(content["forecast_score"])
            ),
            forecast_id=(
                -1 if "forecast_id" not in content else int(content["forecast_id"])
            ),
