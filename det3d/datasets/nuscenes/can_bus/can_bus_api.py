...
class NuScenesCanBus:
    ...
    def __init__(
        self, dataroot: str = "/data/sets/nuscenes", max_misalignment: float = 5.0
    ):
        """
        ...
        if not os.path.isdir(self.can_dir):
            raise Exception(
                "Error: CAN bus directory not found: %s. Please download it from "
        ...
        self.derived_messages = ["meta", "route"]
        self.all_messages = self.can_messages + self.derived_messages

    def print_all_message_stats(self, scene_name: str) -> None:
        """
    ...
    def print_message_stats(self, scene_name: str, message_name: str) -> None:
        """
    ...
    def plot_baseline_route(self, scene_name: str, out_path: str = None) -> None:
        """
    ...
    def plot_message_data(
        ...
        if dimension == -1:
            ...
            raise Exception(
                'Error: Invalid dimension %d for key "%s"!' % (dimension, key_name)
            )
    ...
    def list_misaligned_routes(self, max_misalignment: float = 5.0) -> List[str]:
        """
        ...
        scene_list = list(
            np.unique([f[:10] for f in all_files])
        )  # Get the scene name from e.g. scene-0123_meta.json.
    ...
    def get_pose_and_route(
        self, scene_name: str, print_warnings: bool = True
    ...
    def get_messages(
        self, scene_name: str, message_name: str, print_warnings: bool = True
        ...
        assert message_name in self.all_messages, (
            "Error: Invalid CAN bus message name: %s" % message_name
        )
        ...
        if scene_id in self.can_blacklist:
            ...
            if message_name == "route":
                if scene_id in self.route_blacklist:
                    warnings.warn(
                        "Warning: %s is not well aligned with the baseline route!"
                        % scene_name
                    )
            elif message_name == "vehicle_monitor":
                if scene_id in [419]:
                    warnings.warn(
                        "Warning: %s does not have any vehicle_monitor messages!"
                    )

        # Load messages.
        message_path = os.path.join(
            self.can_dir, "%s_%s.json" % (scene_name, message_name)
        )
