...
class SimAnneal(Annealer):
    ...
    def move(self):
        if random.choice([True, False]):
            if random.choice([True, False]):
                self.state["layers"][0] = self.state["layers"][0] + random.choice(
                    [-1, 1]
                )
            else:
                self.state["layers"][1] = self.state["layers"][1] + random.choice(
                    [-1, 1]
                )
        else:
    ...
    def energy(self):
        ...
        os.system(
            "python  ./tools/train.py configs/{architecture}/{configPath} --seed 0 --work_dir {det_dir}".format(
                architecture=architecture, configPath=configPath, det_dir=self.save_path
            )
        )
        os.system(
        ...
        detection_dataFrame["CLASS"] = detection_dataFrame["CLASS"] + getDetectionNames(
            cohort_analysis
        )
        ...
        for classname in detection_dataFrame["CLASS"]:
            ...
            detection_dataFrame["mFAP_MR"].append(
                logFile["mean_dist_faps_mr"][classname]
            )

        classMetrics = logFile["label_tp_errors"]
        for metric in detection_metrics.keys():
            for classname in detection_dataFrame["CLASS"]:
                detection_dataFrame[detection_metrics[metric]].append(
                    classMetrics[classname][metric]
                )
        ...
        filename = "results/{experiment}/{model}/{dataset}_{architecture}_{model}_{forecast}_{forecast_mode}_{rerank}_tp{tp_pct}_K{K}_{cohort}{static_only}{association_oracle}{jitter}{postprocess}detection_{modelCheckPoint}.csv".format(
            ...
            dataset=dataset,
            architecture=architecture,
            forecast="t{}".format(forecast),
            forecast_mode=forecast_mode,
            rerank=rerank,
            tp_pct=tp_pct,
            K=K,
            jitter="{}jitter_".format(C) if jitter else "",
            cohort="cohort_" if cohort_analysis else "",
            static_only="static_" if static_only else "",
            association_oracle="oracle_" if association_oracle else "",
            postprocess="pp_" if postprocess else "",
            modelCheckPoint=modelCheckPoint,
        )
