...
parser.add_argument("--rootDirectory", default="/media/asghar/media/NUSCENES_DATASET_ROOT_MINI")
...
filename = "results/{experiment}/{model}/{dataset}_{architecture}_{model}_{forecast}_{forecast_mode}_{rerank}_tp{tp_pct}_K{K}_{cohort}{static_only}{association_oracle}{jitter}{postprocess}detection_{modelCheckPoint}.csv".format(
    experiment=experiment, model=model, dataset=dataset, architecture=architecture, forecast="t{}".format(forecast),
    forecast_mode=forecast_mode, rerank=rerank, tp_pct=tp_pct, K=K, jitter="{}jitter_".format(C) if jitter else "",
    cohort="cohort_" if cohort_analysis else "", static_only="static_" if static_only else "",
    association_oracle="oracle_" if association_oracle else "", postprocess="pp_" if postprocess else "",
    modelCheckPoint=modelCheckPoint)
